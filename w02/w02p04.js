    ...
    canvas.addEventListener("click", function (ev) {
        ...
        // Circle mode
            ...
            if (!circleCenter) {
                ...
                circleCenterColor = colorValues[colorSelect.value];
                add_point(positions, mousepos, point_size);
                colors.push(...Array(verts_per_point).fill(colorValues[colorSelect.value]));
                ...
                const radius = length(subtract(circleCenter, mousepos));
                const edgeColor = colorValues[colorSelect.value];

                positions.push(circleCenter);
                colors.push(circleCenterColor);

                for (let i = 0; i <= N; i++) {
                    const angle = (i / N) * 2 * Math.PI;
                    positions.push(add(vec2(radius * Math.cos(angle), radius * Math.sin(angle)), circleCenter));
                    positions.push(circleCenter);
                    positions.push(add(vec2(radius * Math.cos(angle), radius * Math.sin(angle)), circleCenter));
